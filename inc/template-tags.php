...
if ( ! function_exists( 'selfish_posted_by' ) ) :
	...
	function selfish_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'selfish' ),
	...
	function selfish_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'selfish' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'selfish' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'selfish' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'selfish' ),
					array(
						'span' => array(
							'class' => array(),
						),
